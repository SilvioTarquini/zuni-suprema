# STATUS ZUNI SUPREMA — HISTÓRICO

> Registro cronológico completo do projeto, por data, intacto. **Não é lido por
> padrão** — o skill `zuni-continuidade` e qualquer sessão nova leem só
> `STATUS_ZUNI.md`, que tem o estado atual (o que está no ar, aberto ou vigente
> agora). Este arquivo existe para consulta: "quando/por que decidimos X",
> "o que já foi tentado em Y", auditoria de uma frente antiga.
>
> Criado em 14/09/2026 ao separar o `STATUS_ZUNI.md` (que tinha virado um log de
> ~287 KB, lido inteiro a cada sessão) em dois arquivos. Nada foi reescrito nesta
> separação — o conteúdo abaixo é o texto original, só reorganizado por origem.

---

## Digest cronológico antigo ("Última atualização" + cadeia de "Nota anterior")

Formato usado antes da seção "Decisões estratégicas" abaixo ter virado a fonte
primária — cada nota resume, na ordem inversa, o que mudou numa sessão. Preservado
...
## Handoffs e "Lançamentos Recentes" (formato antigo, 14/08 a 20/08/2026)

Bloco pré-"Decisões estratégicas": cada sessão registrava um handoff ou uma lista de
lançamentos recentes, às vezes reaproveitando o mesmo número de seção ("## 2.")
várias vezes com datas diferentes. Preservado como veio.
...
## Fragmentos movidos das seções "Em produção", "Pendências" e "Frente ativa"

Estes trechos ficavam misturados com o estado atual dentro de "Em produção,
funcionando", "Pendências antigas, ainda em aberto" e "Frente ativa" — mas
descrevem algo já concluído/resolvido/datado. Movidos para cá na separação de
14/09/2026, sem edição de conteúdo.

### [11/08/2026] Ajuste de contraste + redesign visual de `chat.html`

**Nota**: a marca "PENDENTE VALIDAÇÃO EM PRODUÇÃO" abaixo é de 11/08/2026.
`chat.html` passou por pelo menos duas reformas visuais depois disso (07/09 —
paleta clara; 14/09 — cabeçalho com Ajuda/Livros ZUNI), sem repetir essa
validação especificamente. Status real não reconfirmado — se for relevante, checar
as cores atuais antes de assumir resolvido ou aberto.
...
- Consolidação do próprio STATUS_ZUNI.md, que virou log em vez de retrato de estado:
  ...
  **✅ Resolvida em 14/09/2026** — é exatamente esta separação em `STATUS_ZUNI.md` +
  `STATUS_ZUNI_HISTORICO.md`.
...
### Mudanças concluídas na frente "Questionários pós-checkout + RAG por tema"
